...
/* 
 	클래스명 => 식별자
 		...
 		3) 키워드 사용x
 		4) 특수문자( _ , $)
 			ex) _클래스명 (언더바가 앞에 있으면 임시 클래스)
 		
 		약속사항 : 대문자로 시작(헝거리언식 표현) ex. FileName
 				 객체명은 변수명과 동일하게 만든다.
 				 ----   ----- (일반변수)
 				(클래스 변수)
...
class Music{
	// 순위, 등락(state, idcrement), 제목, 가수, 앨범, 앨범표지
	int mno;
	String title;
	String singer;
	String album;
...
public class 객체지향프로그램_4 {
	...
	public static void main(String[] args) {
		...
		Music[] music = new Music[200]; // 클래스는 일반 변수처럼 배열 사용 가능
	...
	try {
		...
		for(int i=1;i<=4;i++) {
			...
			//HTML은 tag<>로 이루어짐 => 자바에서는 Element라고 함
